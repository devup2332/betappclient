...
const AdminSidenav = () => {
    ...
    return (
        <div
            className="fixed top-0 left-0 w-full h-full transition-all duration-300 lg:static lg:h-screen lg:6/12 xl:w-3/12 2xl:w-3/12 "
            style={{
        ...
        >
            <div
                className="transition-all bg-bgsidenav duration-300 -left-full font-montserrat absolute top-0 left-0 w-5/6 max-w-xs h-full shadow-card py-7 lg:static left-0 lg:shadow-none lg:h-full lg:w-full lg:max-w-none lg:py-10 overflow-y-auto"
                style={{
                    left: openNav ? "0px" : "-100%",
                }}
            >
                <div className="flex justify-between px-5 xl:px-10">
                    <h1 className="text-3xl text-white font-bold font-montserrat lg:text-2xl">SEPHYRUS</h1>
                    <button onClick={handleCloseSideNav} className=" lg:hidden">
                        <IconClose className="w-5 text-black fill-current" />
                    </button>
                </div>

                <ul className="mt-5 text-xs lg:text-sm">
                    {sidenavOptions.map((opt, index) => {
                        return (
                            <li
                                className="flex gap-3 py-2 px-5 items-center text-white hover:bg-bgbutton cursor-pointer transition-all lg:py-4 xl:px-10"
                                key={index}
